...
for filename in os.listdir('static/pdf_files/'):
    if filename.endswith(".pdf"):
        filenames.append(filename)
        pathname = 'static/pdf_files/'+str(filename)
        path_filename.append(pathname)
        result = extract_data('static/pdf_files/'+filename, read_template)
...
def home():
    return render_template('templates/BS3/invoice_data_extraction.html', posts=pdfFiles, posts1=filenames, posts3=count_values)


@app.route("/verify_invoices")
def about():
    return render_template('templates/BS3/verify_invoices.html', title='About', posts=pdfFiles, posts1=filenames, posts3=count_values, posts4=pdfFiles2)
